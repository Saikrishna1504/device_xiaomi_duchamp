...
class ThermalProfileFragment : Fragment(),
    ...
    // Row icon tints (for coloring active row icon)
    ...
    // Profile value -> card accent color
    private val profileColors = mapOf(
        THERMAL_PROFILE_DEFAULT      to Color.parseColor("#2E7D32"), // green  – Balanced
        THERMAL_PROFILE_MPERFORMANCE to Color.parseColor("#B71C1C"), // red    – Performance
        THERMAL_PROFILE_MBATTERY     to Color.parseColor("#1565C0"), // blue   – Battery Saver
        THERMAL_PROFILE_MGAME        to Color.parseColor("#6A1B9A")  // purple – Game
    )
    ...
    private fun updateUi(activeProfile: Int) {
        ...
        // Card gradient overlay: darker on right side (mimic battery split)
        // We achieve this with a tinted semi-transparent right half using a shape drawable trick —
        // simplest approach is just using a solid color with a darker tint at 70% on the right.
        // For a split look like the battery card, use a layer-list drawable set programmatically:
        ...
        // Active row icon tint (colorize to match card accent)
        val icons = mapOf(
    ...
    /**
     * Build a horizontal gradient drawable that splits light|dark
     * like the MIUI battery card (light green left, dark green right).
     */
